    ...
    class QuestManager
    {
        ...
        public Result AcceptQuest(NetConnection<NetSession> sender, int questId)
        {
            ...
            if (DataManager.Instance.Quests.TryGetValue(questId, out quest))
            {
                ...
                var dbquest = DBService.Instance.Entities.CharacterQuests.Create();//效果等同于TCharacterQuest dbquest = new TCharacterQuest();
                dbquest.QuestID = quest.ID;
                if (quest.Target1 == QuestTarget.None)
                {
                    //没有目标直接完成:接任务同时设置任务为已完成
                ...
                character.Data.Quests.Add(dbquest);//任务绑定到角色上
